...
namespace DropShipping.Services.Implementations
{
    public class OrderService : IOrderService
    {
        ...
        public void UpdateOrder(UpdateOrderDto updateOrderDto)
        {
            ...
            if (orderDb == null)
            {
                throw new ProductNotFoundException($"Order with ID: {updateOrderDto.Id} was not found!");
            }

            User userDb = _userRepository.GetById(updateOrderDto.UserId);
            if (userDb == null)
            {
                throw new ProductNotFoundException($"User with ID: {updateOrderDto.UserId} was not found");
            }
        ...
        public void AddOrder(AddOrderDto addOrderDto)
        {
            ...
            if (userDb == null)
            {
                throw new ProductDataException($"User with ID: {addOrderDto.UserId} does not exist!");
            }

            if (string.IsNullOrEmpty(addOrderDto.FirstName) || string.IsNullOrEmpty(addOrderDto.LastName))
            {
                throw new ProductDataException("Name: Firstname & Lastname are required field!");
            }

            if (string.IsNullOrEmpty(addOrderDto.CardNumber))
            {
                throw new ProductDataException("CardNumber: CardNumber is required field!");
            }

            if (string.IsNullOrEmpty(addOrderDto.Address))
            {
                throw new ProductDataException("Address: Address is required field!");
            }

            if (string.IsNullOrEmpty(addOrderDto.City))
            {
                throw new ProductDataException("City: City is required field!");
            }
        }
        ...
        public void DeleteOrder(int id)
        {
            ...
            if (orderDb == null)
            {
                throw new ProductNotFoundException($"Order with ID: {id} was not found!");
            }
